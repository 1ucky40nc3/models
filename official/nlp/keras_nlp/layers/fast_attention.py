...
"""
# pylint: disable=invalid-name, missing-function-docstring, unexpected-keyword-arg, no-value-for-parameter

import math
import numpy as np
import tensorflow as tf
import official.nlp.keras_nlp.layers.fast_attention_util as util
...
def create_products_of_givens_rotations(dim, seed):
  ...
  return tf.cast(tf.constant(q), tf.float32)
...
def causal_numerator(qs, ks, vs):
  ...
  def grad(res_grad):
    ...
    q_grads = tf.concat(q_grads[::-1], 0)
    k_grads = tf.concat(k_grads[::-1], 0)
    v_grads = tf.concat(v_grads[::-1], 0)
  ...
  def call(self,
    ...
    if cache is not None:
      ...
      if decode_loop_step is not None:
        ...
        key = tf.concat([tf.cast(cache["k"], key.dtype), key], 1)
        value = tf.concat([tf.cast(cache["v"], value.dtype), value], 1)
